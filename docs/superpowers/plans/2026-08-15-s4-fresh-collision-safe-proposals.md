...
- S4 branches from merged S3 plus local post-merge baseline hotfix `8706e1f`; do not add commits to PR #6.
...
**Files:**
- Review only: all changes from `8706e1f..HEAD`
...
```bash
git diff --check 8706e1f..HEAD
git status --short
git diff --stat 8706e1f..HEAD
git diff 8706e1f..HEAD -- \
...
Review every commit and diff from `8706e1f..HEAD`. Verify explicitly:
...
- base `8706e1f` and final S4 commit SHA;
