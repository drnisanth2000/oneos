...
class Scope:
    ...
    def resolve(self, *parts: str | Path) -> Path:
        ...
        base = anchor.resolve()
        if base != anchor:
            raise RedirectedPathError("entity root redirects outside the selected scope")
        candidate = base.joinpath(*map(Path, parts)).resolve()
        if not candidate.is_relative_to(base):
            raise OutOfScopeError("entity path leaves the selected scope")
        return candidate
    ...
    def resolve_stored(self, relative: str | Path) -> Path:
        ...
        if stored.is_absolute() or not stored.parts or stored.parts[0] != self._entity:
            raise OutOfScopeError("stored path belongs to another entity")
    ...
    def vault_relative(self, path: str | Path) -> str:
        ...
        base = self.resolve()
        if not candidate.is_relative_to(base):
            raise OutOfScopeError("path belongs to another entity")
    ...
    def system_path(self, *parts: str | Path) -> Path:
        try:
            ...
            raise RedirectedPathError(str(exc)) from exc
